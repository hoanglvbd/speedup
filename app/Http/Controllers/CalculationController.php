...
class CalculationController extends Controller
{
    ...
    public function exportraw(Request $req)
    {
        ...
        foreach ($data as $key => $user) {
            $results = Result::where('user_id', $user['company_id'])->get()->toArray();
    ...
    public function summaryExcel(Request $req)
    {
        ...
        if ($resultCount == 2) {
            ...
            $row_start = 2;
            foreach ($data as $key => $value) {
                $first = Result::where('user_id', $value['company_id'])->where('number_of_times', 1)->first();
                $second = Result::where('user_id', $value['company_id'])->where('number_of_times', 2)->first();
